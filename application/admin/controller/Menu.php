    ...
    public function save()
    {
        if ($this->request->isPost()) {
            ...
            if ($validate_result !== true) {
                ...
                $pid = $this->auth_rule_model->insertGetId($data);
                if ($pid) {
                    //关联新增子权限
                    /*$controller = explode('/', $data['name'] ?? '');

                    if (isset($controller[2]) && $controller[2] !== 'default') {
                        $prefix = ($controller[0] ?? 'admin') . '/' . ($controller[1] ?? 'menu') . '/';
                        $data = [
                            [
                                'name' => $prefix . 'add',
                                'title' => '添加',
                        ...
                        $this->auth_rule_model::insertAll($data);
                    }*/
