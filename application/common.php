...
function get_category_children($cid)
{
    ...
    $children = Db::name('category')->where(new Where(['path' => ['like', "%,{$cid},%"]]))->select();
...
function get_articles_by_cid($cid, $limit = 10, $where = [], $order = [], $filed = [])
{
    ...
    $ids = Db::name('category')->where(new Where(['path' => ['like', "%,{$cid},%"]]))->column('id');
    ...
    $article_list = Db::name('article')->where(new Where($map))->field($fileds)->order($sort)->limit($limit)->select();
...
function get_articles_by_cid_paged($cid, $page_size = 15, $where = [], $order = [], $filed = [])
{
    ...
    $article_list = Db::name('article')->where(new Where($map))->field($fileds)->order($sort)->paginate($page_size);
...
if (!function_exists('format')) {
     ...
     * @return array
     */
    function format($msg, $code = 0, $data = [], $ext = []): array
    {
        $result = [
            'msg' => $msg,
            'code' => $code,
...
if (!function_exists('array_query')) {
     ...
     * @return array
     */
    function array_query(array $data): array
    {
